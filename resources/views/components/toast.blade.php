        ...
        <div :class="toast.type === 'danger' ? 'toast-card is-danger' : 'toast-card is-success'">
            <div class="flex items-start gap-3 px-4 py-3">
                <div class="min-w-0 flex-1">
                    <p class="text-sm font-semibold text-navy" x-text="toast.type === 'danger' ? 'Something went wrong' : 'Saved'"></p>
                    <p class="mt-0.5 text-sm text-text-2" x-text="toast.message"></p>
                </div>
                <button type="button" class="text-text-2 hover:text-navy" @click="remove(toast.id)" aria-label="Dismiss">
                    ×
                </button>
            </div>
        </div>
