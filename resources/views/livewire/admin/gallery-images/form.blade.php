    ...
    <form wire:submit="save" class="space-y-6">
        <x-card>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <x-field label="Label" name="label"><input wire:model="label" class="w-full rounded-btn border border-border px-3 py-2.5 text-sm"></x-field>
                <x-field label="Alt text" name="alt_text"><input wire:model="alt_text" class="w-full rounded-btn border border-border px-3 py-2.5 text-sm"></x-field>
            </div>
        </x-card>
        <x-card>
            <h3 class="mb-4 font-serif text-xl text-navy">Media</h3>
            <x-field label="Image" name="image"><input type="file" wire:model="image" accept="image/*" class="block w-full text-sm"></x-field>
            @if ($existingImage)
